...
class Paris6k(Dataset):
    ...
    # 输入文件夹路径，返回文件列表
    # paris文件夹多一层文件夹
    def dir_2_filelist(self, d):
        names = os.listdir(d)
        paths = []  # 所有图片路径
        for name in names:
            files = os.listdir(os.path.join(d, name))
            for file in files:
                if file in self.destroy:
                    continue
                paths.append(os.path.join(d, name, file))
        return paths

    def get_filelist(self):
        return self.dir_2_filelist(self.datadir)
    ...
    def get_query(self, dir):
        ...
        for i in range(len(self.query_list)):
            for j in range(1, 6):
                ...
                # goodfile = os.path.join(dir, self.query_list[i] + '_' + str(j) + '_good.txt') # 该文件夹都是空的
    ...
    def evaluate(self):
        ...
        querys, q_v = self.get_query(self.gtdir)
        sum_ap = 0.0
        n = 0
